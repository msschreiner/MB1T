...
correlation_test_retest_sampled_trials <- function(dataset,num_slices=1) {
  #filter dataset
    current_dataset <- dataset %>% group_by(Subject_Unique,Session,Condition) %>% slice_sample(n = num_slices) %>%
    summarise(mean_lt=mean(LT)) %>% pivot_wider(names_from = Condition,values_from = mean_lt) %>% mutate(Diff=IDS-ADS) %>%
    select(-(c("ADS","IDS"))) %>%  pivot_wider(names_from = Session,values_from = Diff, names_prefix = "Diff_") %>% ungroup()
...
Trials = seq(12,16) # Select the number of required trials

num_slices = c(2,4,6,8) # Select the number of needed trial pairs for inclusion


Outcome_sim<-expand.grid(N = N,Trials = Trials,num_slices = num_slices,r=NA)


Outcome_sim$r[Outcome_sim$Trials==2&Outcome_sim$num_slices==2]
...
for (t in Trials) {
  ...
  all_agg_subjects_paired_retest_atleast_12_trials<-all_agg_subjects_paired_retest %>% filter(total_trial_n_session_1>=t&total_trial_n_session_2>=t)
  all_subjects_atleast_12_trials<- data_clean%>% filter(Subject_Unique %in% all_agg_subjects_paired_retest_atleast_12_trials$Subject_Unique)

  
  correlation<-replicate(100,correlation_test_retest_sampled_trials(all_subjects_atleast_12_trials,n))
  
  Outcome_sim$r[N==seq(1:100)&Outcome_sim$Trials==t&Outcome_sim$num_slices==n]<-correlation
    
  }
 
}

Outcome_sim_sum<-Outcome_sim %>% group_by(Trials,num_slices) %>% summarise(r_mean=mean(r),r_sd=sd(r))
...
p6.12 <- ggplot(Outcome_sim_sum[Outcome_sim_sum$Trials==12,],aes(x=num_slices,y=r_mean))+
  geom_hline(yintercept=0,linetype="dashed")+
  geom_point(size=4)+
  xlab("Number of randomly \n selected trial pairs")+
  ylab("Mean Pearson correlation coefficient")+
  ylim(-0.1,1)+
  scale_x_continuous(breaks=c(2,4,6,8))+        theme_cowplot()

p6.16 <- ggplot(Outcome_sim_sum[Outcome_sim_sum$Trials==16,],aes(x=num_slices,y=r_mean))+
  geom_hline(yintercept=0,linetype="dashed")+
  geom_point(size=4)+
  xlab("Number of randomly \n selected trial pairs")+
  ylab("Mean Pearson correlation coefficient")+
...
plot_grid(p6.12,p6.16,labels=c("A","B"))
```
