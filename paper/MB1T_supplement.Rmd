...
```{r fig1, fig.cap="Distribution of participant age for each lab and method. Method is highlighted by color and  language status is indicated by line type (solid = North American English; dashed = non-North American English).", fig.align="center"}
...
```{r fig2, fig.cap = "Infants' preference in Session 1 and Session 2 with individual data points and regression lines color-coded by method (CF, ET, or HPP). Results are plotted separately for North American English-learning infants (right panel) and infants learning other languages and dialects (left panel)."}
...
```{r fig4, fig.cap = "Forest plot of test-retest reliability effect sizes. Each row represents Fisher's z transformed correlation coefficient and 95% CI for a given lab and method (HPP, ET, and CF). The black diamond represents the overall estimated effect size from the mixed-effects meta-analytic model." }
...
```{r fig7, fig.height=3.2,fig.cap = "Pearson correlation coefficient with increasingly strict trial-level inclusion criteria. The x-axis depicts the required number of overall valid trials in both Session 1 and Session 2. Dots represent corresponding correlation coefficients, with 95% CIs. The sample size is shown above each dot."}
...
num_slices = c(1,2,3,4,5,6,7,8) # Select the number of resampled trial pairs
...
write.csv(simulation_outcome, here("data","processed", "resampling_retest_correlation_simulation_outcome.csv"),row.names=FALSE)
```
...
For example, for the sample with the strictest inclusion criterion (all 16 trials present, n=24), test-retest correlations start near zero when including only 2 trials (r = `r simulation_outcome_summarized %>% filter(sample_size==24 & num_slices==1) %>% pull(r_mean) %>% round(2)`) but increase steadily to their peak once resampling all 16 total trials (r = `r simulation_outcome_summarized %>% filter(sample_size==24 & num_slices==8) %>% pull(r_mean) %>% round(2)`).
...
```{r fig10, fig.cap="Correlations in average looking time (in s) between Session 1 and 2 by item.", fig.align="center",fig.height = 16, fig.width = 12}
corr_lt_p3
```
...
We also investigated whether preferential looking behavior varied as a function of infants' tendencies to look for longer or shorter periods of time on average across all stimuli. We found no evidence for a correlation between infants' average looking time and the magnitude of IDS preference (Figure 11), either in Session 1, `r apa_print(session1_lt_pref_correlation)$full_result`, or in Session 2, `r apa_print(session2_lt_pref_correlation)$full_result`.

```{r fig11, fig.cap="Correlations between average looking time (in s) and IDS preference in (A) Session 1 and (B) Session 2.", fig.align="center"}
...
```{r  fig12, fig.height=4,fig.cap="Correlations between average looking time (in s) and IDS preference for infants with shorter looking (left panel) and longer looking (right panel).", fig.align="center"}
all_agg_subjects_paired_retest <- all_agg_subjects_paired_retest %>%
...
data_clean <- data_clean %>%
  ungroup() %>%
  group_by(Subject,Subject_Unique,Lab,Session) %>%
  mutate(LT_previous_trial = dplyr::lag(LT,order_by=trial_num),
         log_lt_previous_trial = dplyr::lag(log_lt,order_by=trial_num)) %>%
  ungroup() %>%
  group_by(Subject_Unique) %>%
  mutate(
    LT_previous_trial_c = LT_previous_trial-mean(LT_previous_trial,na.rm=TRUE),
    log_lt_previous_trial_c = log_lt_previous_trial-mean(log_lt_previous_trial,na.rm=TRUE),
    trial_num_c=trial_num-mean(trial_num,na.rm=TRUE),
    )

#fit a mixed-effects model predicting looking time from trial number and previous trial looking time
#more complex random effects structures with additional random slopes have singular fit
previous_trial_lt_model <- lmer(LT ~ LT_previous_trial_c+trial_num_c+(1+trial_num_c|Subject_Unique)+(1|Lab),data=data_clean)
#similar results when predicting using log lt
# previous_trial_log_lt_model <- lmer(log_lt ~ log_lt_previous_trial_c+trial_num_c+(1+trial_num_c|Subject_Unique)+(1|Lab),data=data_clean)
# summary(previous_trial_log_lt_model)
```

```{r fig14, fig.cap="Relation between (log-based) looking time and (log-based) looking time on the previous trial. Each panel represents one of the trial numbers of the experiment (starting from the 2nd trial). The blue line represents a linear fit with 95% CI error bands.", fig.align="center"}
ggplot(filter(data_clean,trial_num>1),aes(log_lt_previous_trial,log_lt))+
  geom_point(alpha=0.3)+
  geom_smooth(method="lm")+
  facet_wrap(~trial_num,nrow=3)+
  theme_cowplot()+
  theme(axis.text = element_text(size=10))+
  xlab("Log Looking Time on Previous Trial")+
  ylab("Log Looking Time")
```
...
`r apa_print(previous_trial_lt_model)$full_result$LT_previous_trial_c` (see Figure 14).
...
```{r fig15, fig.cap="IDS preference (IDS-ADS) for pairs of (adjacent) trials, for Session 1 (left panel) and Session 2 (right panel). Error bars represent +1/-1 SEs. The blue line represents a loess fit through the data with 95% confidence bands.", fig.align="center"}
...
```{r}
agg_by_subj_paired_preference <- agg_by_subj_paired_preference %>%
  ungroup() %>%
  group_by(Subject,Subject_Unique,Lab,Session) %>%
  mutate(Diff_previous_trial_pair = dplyr::lag(Diff,order_by=trial_pair)) %>%
  ungroup() %>%
  group_by(Subject_Unique) %>%
  mutate(
    Diff_previous_trial_pair_c = Diff_previous_trial_pair-mean(Diff_previous_trial_pair,na.rm=TRUE)
    )

#fit a mixed-effects model predicting looking time from trial number and previous trial looking time
#more complex random effects structures do not converge
previous_trial_diff_model <- lmer(Diff ~ Diff_previous_trial_pair_c+(1+Diff_previous_trial_pair_c|Subject_Unique)+(1|Lab),data=agg_by_subj_paired_preference)
```
...
We did not find a significant effect of IDS preference on the previous trial pair, `r apa_print(previous_trial_diff_model)$full_result$Diff_previous_trial_pair_c`.
...
model_formulas <- c(
  "log_lt ~ 1+(1|Subject_Unique)",
  "log_lt ~ 1+trial_num_c+(1+trial_num_c|Subject_Unique)",
  "log_lt ~ 1+trial_num_c+condition_c+(1+trial_num_c+condition_c|Subject_Unique)",
  "log_lt ~ 1+trial_num_c+condition_c+session_c+(1+trial_num_c+condition_c+session_c|Subject_Unique)",
  "log_lt ~ 1+trial_num_c+condition_c+session_c+average_age+NAE+method_hpp_et+method_hpp_cf+days_between_sessions+(1+trial_num_c+condition_c+session_c|Subject_Unique)"
)

#We also considered the model formula including looking on the previous trial:

#"log_lt ~ 1+session_c+condition_c+trial_num_c+LT_previous_trial_c+average_age+NAE+method_hpp_et+method_hpp_cf+days_between_sessions+(1+session_c+condition_c+trial_num_c|Subject_Unique)"

#However, we did not include looking time on the previous trial in the final model for three reasons:
#1. The model including the by-participant random slope for previous looking time did not converge, which complicated the variance decomposition.
#2. Including the fixed effect of looking time on the previous trial reduced the number of observations by several hundred observations (due to missing trials), which affects the comparison across models.
#3. Even when including this factor, the variance decomposition changed only slightly from the model without this factor.
...
model_data <- data.frame(model_formulas = model_formulas,formula_name=c("Random Intercept Only","Trial Num.","Trial Num. + Condition","Trial Num. + Condition + Session", "Full Model")) %>%
...
full_model_collinearity_check <- check_collinearity(model_data$model[[5]])
```

```{r results="asis",warning=FALSE,message=FALSE}
# Coefficients for full linear mixed-effects model
full_model <- model_data$model[[5]]
#get table output
apa_results_full_model <- apa_print(full_model)
#rename term in table
apa_results_full_model$table$term <- c(
  "Intercept",
  "Trial Number",
  "Condition (ADS vs. IDS)",
  "Session",
  "Age (averaged across sessions)",
  "NAE status",
  "Method (HPP vs. ET)",
  "Method (HPP vs. CF)",
...
#print table
apa_table(
  apa_results_full_model$table
  , caption = "Results of the final linear mixed-effects model predicting trial-by-trial log looking time from the focal within- and between-participant predictors of interest."
)
```
...
Table 7 shows the results of this final model - note that no evidence of multi-collinearity was observed (all VIFs < `r ceiling(max(full_model_collinearity_check$VIF))`).
We then used the R packages `r2mlm` [@R-r2mlm] and `performance` [@R-performance] to decompose variance in looking behavior explained within each model.
...
We find that ultimately about a third of the variance (~0.3) could be attributed to participant grouping in the data once including the within-participant predictors related to trial number, condition, and session (see the `performance` package for details on the differences between adjusted and unadjusted ICCs; in brief, the unadjusted ICC also adds the fixed-effects variances to the denominator in the ICC equation).

```{r results="asis",warning=FALSE,message=FALSE}
icc_table <- model_data %>%
  select(formula_name,icc) %>%
  unnest(icc) %>%
  select(-ICC_conditional) %>%
  rename(`Model Predictors` = formula_name, 
         `ICC (Adjusted)` = ICC_adjusted,
         `ICC (Unadjusted)` = ICC_unadjusted)

papaja::apa_table(icc_table, 
                  caption = "ICC results for the linear mixed-effects model predicting trial-by-trial log looking time from within- and between-participant predictors of interest.", 
                  format.args = list(digits = 2))
```

```{r fig16, fig.height = 7,fig.cap="Total R-squared measures for sequentially more complex models predicting trial-by-trial log looking time. Each bar represents the proportion total variance explained by random effects (mean variation, slope variation) and fixed effects (both within and between) for each model. We sequentially add within-subject predictors (trial number, condition, session) to the intercept-only model. The full model additionally includes a set of focal between-participant predictors (see Table 7). See the r2mlm package for further details on the R-squared measures.", fig.align="center"}

#write function to extract the decompositions and convert to a data frame
convert_decompositions_to_df <- function(r2mlm_object) {
  decomposition_matrix <- as.matrix(r2mlm_object$Decompositions)
  #as.data.frame.matrix is key here for preserving dimensional structure
  df <- as.data.frame.matrix(decomposition_matrix)
  #make row.names from matrix list a new column in df
  df$variance_component <- row.names(decomposition_matrix)
  return(df)
}

variance_decomposition_df <- model_data %>%
  mutate(decompositions = purrr::map(r2mlm,convert_decompositions_to_df)) %>%
  select(formula_name,decompositions) %>%
  unnest(cols=c(decompositions)) %>%
  ungroup() %>%
  mutate(
    variance_component = factor(variance_component,levels=c("sigma2","slope variation","mean variation","fixed, within","fixed, between"))
  )

ggplot(variance_decomposition_df,aes(x=formula_name,y=total,fill=variance_component))+
  geom_bar(stat="identity",position="stack",color="black")+
  theme_cowplot()+
  xlab("Model")+
  ylab("Total Variance Explained")+
  #rotate x-axis labels 90 degrees and shift values a little left
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))+
  scale_x_discrete(limits = c("Random Intercept Only","Trial Num.","Trial Num. + Condition","Trial Num. + Condition + Session", "Full Model"),
                   labels=c("Random Intercept Only","Trial Num","Trial Num+Condition","Trial Num+Condition+Session", "Full Model"))+
  scale_fill_manual(name="Variance Component",
                    limits=c("sigma2","slope variation","mean variation","fixed, within","fixed, between"),
                    values=c("sigma2"="white","slope variation"="#92c5de","mean variation"="#0571b0","fixed, within"="#ca0020","fixed, between"="#f4a582"),
                    labels=c("Residual","Slope Variation (within)","Intercept Variation (between)","Fixed Slopes, Within","Fixed Slopes, Between"))+
  guides(alpha = "none")
```
