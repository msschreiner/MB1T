...
# S6.3. Predictiveness of IDS preferences across trials within a trial
...
#add a column representing the Diff of the previous trial_pair within each Subject and Session
agg_by_subj_paired_preference <- agg_by_subj_paired_preference %>%
  ungroup() %>%
  group_by(Subject,Subject_Unique,Lab,Session) %>%
  mutate(Diff_prev = dplyr::lag(Diff,order_by=trial_pair)) %>%
  #also compute mean diff
  mutate(mean_Diff = mean(Diff,na.rm=TRUE)) %>%
  mutate(Diff_from_mean = Diff-mean_Diff,
         Diff_from_prev = Diff-Diff_prev)


#predict Diff from Diff_prev
m <- lmer(Diff ~ Diff_prev*trial_pair + (1|Subject) + (1+Diff_prev|Lab), data = agg_by_subj_paired_preference)
summary(m)

ggplot(agg_by_subj_paired_preference,aes(Diff_prev,Diff))+
  geom_point()+
  geom_smooth(method="lm")+
  facet_wrap(~trial_pair)
...
ggplot(agg_by_subj_paired_preference,aes(x=trial_pair,y=Diff))+
  ...
  xlab("Trial pair")+
  ylab("Average IDS preference (Diff)")+
  theme_cowplot()+
  facet_wrap(~Session)
