...
ggplot(all_agg_subjects_paired_retest,aes(Age,color=Method))+
  ...
  geom_label(aes(label=paste0("N = ",N)),x=300,y=0.0125,size=2)+
...
This effect appears to be mainly driven by older infants showing some evidence of test-retest reliability for the headturn preference procedure, *r* = `r round(filter(coef_table_method_age, Method=="HPP"&age_bin=="older")$cor,2)`, *p* = `r round(filter(coef_table_method_age, Method=="HPP"&age_bin=="older")$pvalue2,2)` (see Figure 3B).
...
We tested whether method (eye-tracking vs. central fixation vs. headturn preference procedure) moderated test-retest reliability by fitting a linear mixed-effects model predicting IDS preference in Session 2 from IDS preference in Session 1 (mean-centered), Method (dummy-coded, with central fixation as the reference level) and their interaction.
The model included a by-lab random intercept and a by-lab random slope for IDS preference in Session 1.
We found no evidence that Method influenced test-retest reliability as indicated by the interaction between IDS preference in Session 1 and age, ${\chi}^2$=`r round(overall_chisq_test_method$Chisq[4],2)`, *p*=`r printp(overall_chisq_test_method[4,3])`. There was no significant relationship between IDS preference for session 1 and session 2 for each method considered separately (central fixation:$\beta$=`r round(coef_table_6$estimate[1],3)`, *SE*=`r round(coef_table_6$SE[1],2)`, *p*=`r printp(coef_table_6$pvalue[1])`; HPP: $\beta$=`r round(coef_table_6$estimate[2],3)`, *SE*=`r round(coef_table_6$SE[2],2)`, *p*=`r printp(coef_table_6$pvalue[2])`; eye-tracking: $\beta$=`r round(coef_table_6$estimate[3],3)`, *SE*=`r round(coef_table_6$SE[3],2)`, *p*=`r printp(coef_table_6$pvalue[3])`)
...
```{r sfig7, fig.cap = "Pearson correlation coefficient with increasingly strict trial-level inclusion criteria. The x-axis depicts the required number of overall valid trials in both session 1 and session 2. Dots represent corresponding correlation coefficients, with 95 percent CIs. The sample size is shown above each dot."}
...
We found little evidence of distinct clusters emerging from these groupings: the clusterings ranging from *k*=2 (2 clusters) to *k*=4 (4 clusters) appear to mainly track  whether participants are approximately above or below the mean looking time difference for test session 1 and test session 2 (Figure 8A).
...
```{r fig8, fig.cap = " (A) Results from the k-means clustering analysis of IDS preference (based on average log looking times) in session 1 and 2 for different numbers of k  and (B) the corresponding elbow plot of the total within-cluster sum of squares. In (A), points represent indvidual participants' magnitude of looking time difference at test sessions 1 (x-axis) and 2 (y-axis). The solid line indicates no preference for IDS vs. ADS, the dotted lines indicate mean IDS preference at test session 1 and 2, respectively. Colors indicate clusters from the k-means clustering for different values of k."}
p1 <- 
...
```{r sfig9, fig.cap = "Correlation between the number of trials contributed in Session 1 and Session 2. Each data point represents one infant. Colored lines represent linear fits for each method."}
# ordering levels of method as in language background plot (Figure 2 of Supplementary)
...
We found a strong positive correlation between number of trials contributed during the first and the second session `r cor_trials_out$full_result` (Figure 9). 
...
First, we found similar correlations in average looking time to IDS stimuli in Session 1 and 2, `r apa_print(correlation_IDS_LT)$full_result`, and ADS stimuli in Session 1 and 2, `r apa_print(correlation_ADS_LT)$full_result` (Figure 10B).
...
Item-level looking in Session 2 was related to item-level looking in Session 1, `r apa_print(item_lt_relationship)$full_result[["LT_Test"]]` (Figure 10C).
...
```{r  sfig10, fig.cap="Correlations in average looking time (in s) between Session 1 and 2 (A) overall, (B) by condition, and (C) by item.", fig.align="center",fig.height = 16, fig.width = 12}
...
#not plotted
fig11 <- ggplot(agg_by_subj_item_paired_pref ,aes(Diff_1,Diff_2))+
  geom_point()+
  geom_smooth(method="lm",color="black")+
...
# S11. Overall looking times and test-retest IDS preference

## S11.1. Correlations between overall looking time and IDS preference
...
```{r  sfig11, fig.cap="Correlations between average looking time (in s) and IDS preference in (A) Session 1 and (B) Session 2.", fig.align="center"}
...
## S11.2. Does average looking time moderate test-retest reliability?
...
- marginal effect
- direction of the effect is consistent with a slight increase in test-retest reliability as average looking time increases (Figure 12), but no evidence for robust differences between long and short lookers.

```{r  sfig12, fig.cap="Correlations between average looking time (in s) and IDS preference in (A) Session 1 and (B) Session 2.", fig.align="center"}
all_agg_subjects_paired_retest <- all_agg_subjects_paired_retest %>%
