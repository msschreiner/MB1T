...
To address this question, we conducted a simulation in which, for each subset of participants defined based on the number of trials needed for inclusion (2-16; see Figure 7), we resampled increasingly larger numbers of trials for the given subset of participants (in increments of pairs of IDS and ADS trials, 1-8 trial pairs) and computed the average resulting test-retest correlation (Figure 8).
...
For example, for the sample with the strictest inclusion criterion (all 16 trials present, n=24), test-retest correlations start very low when including only 2 trials (r = `r simulation_outcome_summarized %>% filter(sample_size==24 & num_slices==1) %>% pull(r_mean) %>% round(2)`) but increases steadily to its peak once resampling 16 total trials (r = `r simulation_outcome_summarized %>% filter(sample_size==24 & num_slices==8) %>% pull(r_mean) %>% round(2)`).
This result also rules out that the increase in test-retest correlation with stricter inclusion criteria observed in Figure 7 is solely a function of selecting a set of participants with traits that support more reliable measurement --- increasing the number of trials is critical to obtaining reliable measurements within each subset of participants.
...
model_data <- data.frame(model_formulas = model_formulas,formula_name=c("Trial Number","Trial Number + Condition","Trial Number + Session + Condition","Trial Number + Session + Condition+\nPrevious Trial Looking", "Full Model","Full Model Minus")) %>%
...
check_collinearity(model_data$model[[5]])
```
