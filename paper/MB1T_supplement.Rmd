...
```{r  sfig11, fig.cap="Correlations between average looking time (in s) and IDS preference for infants with (A) shorter looking and (B) longer looking.", fig.align="center"}
all_agg_subjects_paired_retest <- all_agg_subjects_paired_retest %>%
  mutate(looking_bin = cut(mean_lt_1, breaks = 2, labels = c("A shorter looking"," B longer looking")))
