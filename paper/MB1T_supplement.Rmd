...
```{r fig7, fig.cap = "Pearson correlation coefficient with increasingly strict trial-level inclusion criteria. The x-axis depicts the required number of overall valid trials in both Session 1 and Session 2. Dots represent corresponding correlation coefficients, with 95% CIs. The sample size is shown above each dot."}
...
For each level (participant group X number of trial pairs), we conducted 100 simulations (Figure 8).
...
An alternative possibility is a certain number of observations per participant is helpful for generating robust estimates (especially given that we resample with replacement in the simulation, i.e. for participants with small numbers of trials, we are essentially resampling the same small number of noisy observations repeatedly).
We hope this simulation-based investigation of changes in test-retest reliability with increasing numbers of observations sets the stage for further data-driven explorations of factors shaping reliable measurement in looking-time based infant studies.
...
```{r  fig12, fig.cap="Correlations between average looking time (in s) and IDS preference for infants with shorter looking (left panel) and longer looking (right panel).", fig.align="center"}
all_agg_subjects_paired_retest <- all_agg_subjects_paired_retest %>%
...
model_formulas <- c(
  ...
  "log_lt ~ 1+session_c+condition_c+trial_num_c+LT_previous_trial_c+(1+session_c+condition_c+trial_num_c|Subject_Unique)",
...
model_data <- data.frame(model_formulas = model_formulas,formula_name=c("Trial Num.","Trial Num. + Condition","Trial Num. + Session + Condition","Trial Num. + Session + Condition + Prev. Trial Looking", "Full Model")) %>%
...
apa_results_full_model$table$term <- c(
  ...
  "Trial Number",
  "Session",
  "Condition (IDS vs. ADS)",
  "Previous Trial Looking Time",
...
apa_table(
  ...
  , caption = "Results of the final linear mixed-effects model predicting trial-by-trial log looking time from all within- and between-participant predictors of interest."
)
```
