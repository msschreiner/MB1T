...
To test whether the results were different for a specific method, we calculated the Pearson correlation coefficients and the multilevel analyses for the three different methods, HPP, central fixation and eye-tracking, separately (see Table 4). There was no evidence that method moderated test-retest reliability. Neither the Pearson correlation coefficients nor the coefficients of the multilevel analysis were significant, all *p*-values > .286. In series of planned secondary analyses, we found that participant age, language background, and time between test sessions also did not moderate the relationship between IDS preference in session 1 and session 2 (see Suppmentary Materials, S2). Taken together, we find no significant evidence of test-retest reliability across our preregistered analyses.
