...
In particular, infants’ word segmentation abilities [@floccia2016british; @schreiner2017listen; @singh2009influences, @thiessen2005infant] and their learning of word-object associations [@graf2013infant; @ma2011word] seems to be enhanced in the context of IDS. 
...
Nevertheless, the overall effect size *d* = 0.35 was much smaller than the meta-analytic effect size of *d* = 0.67 reported by @dunst2012preference.
...
For example, @colombo1988infant used a paired comparison task, in which infants were familiarized with a stimulus and for the test trials presented with the familiarized and a novel stimulus side-by-side. 
...
Nevertheless, the results showed that reliability was extremely variable across the different experiments and labs and overall low (meta-analytic *r* = .07).
...
Hence, it remains an open question whether the same measures that produce larger effect sizes at the group-level also have higher test-retest reliability for individual infants [@byers2021six]. 
...
Prior to the start of data collection, we preregistered the current study on the Open Science Framework (https://osf.io/v5f8t; see S1 in the Supplementary Materials for further details). 
...
First, we examined infants' preferences for IDS in both sessions. Two two-samples t-tests revealed that the children in Session 1, `r apa_print(ttestSession1)$statistic`, and in Session 2, `r apa_print(ttestSession2)$statistic`, showed a preference of IDS over ADS (see Table 2 for the average looking times in each session), replicating the main finding from MB1.
In the first session, `r sum(all_agg_subjects_paired_s1$Diff>0)/length(all_agg_subjects_paired_s1$Diff)*100`% of infants showed a preference for IDS, and in the second session, `r sum(all_agg_subjects_paired_s2$Diff>0)/length(all_agg_subjects_paired_s2$Diff)*100`% of infants showed a numerical preference for IDS. 
...
There was no significant difference in the magnitude of infants’ preference between the two sessions, $\beta$=`r round(diff_session_coefs[2,1],3)`, *SE*=`r round(diff_session_coefs[2,2],2)`, *p*=`r printp(diff_session_coefs[2,5])`.
...
The results revealed no significant relationship between IDS preference in Session 2 and Session 1 (Table 2). 
...
To test whether the results were different for a specific method, we calculated the Pearson correlation coefficients and the multilevel analyses for HPP, central fixation and eye-tracking separately (see Table 4). 
There was no evidence that method moderated test-retest reliability. 
Neither the Pearson correlation coefficients nor the coefficients of the multilevel analysis were significant, all *p*-values > .286.
In planned secondary analyses, we found that participant age, language background, and time between test sessions also did not moderate the relationship between IDS preference in session 1 and session 2 (see Suppmentary Materials, S2).
...
Applying stricter criteria - and thereby increasing the number of test trials - increased reliability numerically from r = `r round(min(correlations),2)` to r = `r round(max(correlations),2)` (Figure 1). 
In part due to a decrease in sample size, only one of these correlations was statistically significant (namely when requiring a minimum of 6 trial pairs): 2 valid trial pairs, `r print_test_retest_correlation(paired_retest_df_by_trial_exclusions,minimum_trials=2)`; 4 valid trial pairs, `r print_test_retest_correlation(paired_retest_df_by_trial_exclusions,minimum_trials=4)`; 6 valid trial pairs, `r print_test_retest_correlation(paired_retest_df_by_trial_exclusions,minimum_trials=6)`; 8 valid trial pairs - all trials in both sessions - `r print_test_retest_correlation(paired_retest_df_by_trial_exclusions,minimum_trials=8)`. 
