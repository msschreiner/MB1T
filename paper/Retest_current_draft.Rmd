...
To investigate the test-retest reliability of each specific method, we computed Pearson correlation coefficients and the same mixed-effects model described above for HPP, central fixation and eye-tracking separately (see Table 4). 
...
In planned secondary analyses, we found that method, participant age, language background, and time between test sessions did not moderate the relationship between IDS preference in session 1 and session 2 (see Suppmentary Materials, S2).
...
Infants of the original MB1 project were retested on a reversed order of stimuli in order to assess if their listening pattern would be similar to that of their initial assessment. 
...
In other words, we were unable to detect any stable individual differences of infants' preference for IDS. 
This finding is consistent with other research suggesting low test-retest reliability in other infant paradigms [@cristia2016test]. 
...
In the case of studies in the field of developmental science, where there are many practical restrictions in collecting large samples of infants (e.g., birth rate in the area, restricted lab capacities, budget restrictions), a strict drop-out criterion may often not be easy --- if even possible at all --- to implement. 
Note that studies in developmental science already have above average drop out rates [@miller2017developmental]. In addition, drop out may not be random, and so having high drop out rates can further limit the generalizability of a study. 
