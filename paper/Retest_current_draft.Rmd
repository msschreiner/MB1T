---
title             : "Limited evidence of test-retest reliability in infant-directed speech preference in a large pre-registered infant experiment"
...
Previous attempts to address the reliability of measurements have typically been limited to adult populations [@hedge2018reliability], or have been conducted with small sample sizes [e.g., @houston2007assessing].
...
For example, @marimon2022 found no evidence for test-retest reliability when testing infants' prosodic preferences using the HPP method.
...
df_all_exclude_counts <- df_all_exclude %>%
  ...
  group_by(Subject_Unique,preterm) %>%
...
#exclude preterms
#excludePreterm<-unique(df_all$Subject_Unique[df_all$preterm=="preterm"])
...
`r sum(df_all_exclude_counts$session_error_type=="preterm exclusion")` participants were excluded for being preterm (defined as a gestation time of less than 37 weeks).
`r sum(df_all_exclude_counts$session_error_type %in% c("experimenter error","experiment error"))` participants were excluded due to session errors involving an experimenter error (e.g., inaccurate coding or presentation of retest stimuli on the first test session). 
Individual trials were excluded if they were marked as trial errors, i.e., if the infant was reported as fussy, an experimental or equipment error occurred, or there was parental interference during the task (e.g., if the parent spoke with the infant during the trial).
Trials were also excluded if the minimum looking time of 2 s was not met (`r round(sum(all_agg_subjects_paired$N_below_threshold_lt,na.rm=TRUE)/sum(all_agg_subjects_paired$total_trial_n,na.rm=TRUE),3) * 100`% of the remaining trials). 
