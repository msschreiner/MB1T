...
\begin{figure}
\centering
\includegraphics{MB1T_supplement_files/figure-latex/fig4-1.pdf}
\caption{\label{fig:fig4}Forest plot of test-retest reliability effect sizes. Each row represents Fisher's z transformed correlation coefficient and 95\% CI for a given lab and method (HPP, ET, and CF). The black diamond represents the overall estimated effect size from the mixed-effects meta-analytic model.}
\end{figure}
...
\caption{\label{tab:unnamed-chunk-27}Coefficient estimates from a linear mixed-effects model predicting Log LT IDS preference in Session 2.}
...
{\centering \includegraphics[width=5in]{MB1T_supplement_files/figure-latex/fig5-1} 

}

\caption{IDS preferences (based on average log-looking times) of both sessions plotted against each other for each inclusion criterion. \textit{n} indicates the number of included infants, \textit{r} is the Pearson correlation coefficient as the indicator for reliability.}\label{fig:fig5}
\end{figure}
...
{\centering \includegraphics[width=5in]{MB1T_supplement_files/figure-latex/fig6-1} 

}

\caption{IDS preferences (based on proportion IDS looking) of both sessions plotted against each other for each inclusion criterion. n indicates the number of included infants, r is the Pearson correlation coefficient as the indicator for reliability.}\label{fig:fig6}
\end{figure}
...
\caption{\label{tab:unnamed-chunk-29}Coefficient estimates from a linear mixed-effects model predicting IDS preference (based on proportion IDS looking) in Session 2.}
...
\hypertarget{s6.2.-simulation-with-subset-of-participants}{%
\subsection{S6.2. Simulation with subset of participants}\label{s6.2.-simulation-with-subset-of-participants}}
...
For example, for the sample with the strictest inclusion criterion (all 16 trials present, n=24), test-retest correlations start very low when including only 2 trials (r = 0.04) but increases steadily to its peak once resampling 16 total trials (r = 0.34).
...
\begin{figure}
\centering
\includegraphics{MB1T_supplement_files/figure-latex/fig9-1.pdf}
\caption{\label{fig:fig9} (A) Results from the k-means clustering analysis of IDS preference (based on average log looking times) in Session 1 and 2 for different numbers of k, and (B) the corresponding elbow plot of the total within-cluster sum of squares. In (A), points represent indvidual participants' magnitude of looking time difference at Sessions 1 (x-axis) and 2 (y-axis). The solid line indicates no preference for IDS vs.~ADS, the dotted lines indicate mean IDS preference at Session 1 and 2, respectively. Colors indicate clusters from the k-means clustering for different values of k.}
\end{figure}
...
{\centering \includegraphics{MB1T_supplement_files/figure-latex/fig10-1} 

}

\caption{Correlations in average looking time (in s) between Session 1 and 2 by item.}\label{fig:fig10}
\end{figure}
...
We then used the R packages \texttt{r2mlm} and \texttt{performance} to decompose variance in looking behavior explained within each model.
...
We find that ultimately about a third of the variance (\textasciitilde0.3) could be attributed to participant grouping in the data once including the within-participant predictors related to trial number, session, and condition.
