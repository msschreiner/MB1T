...
We fit a linear mixed-effects model predicting average IDS preference from Session (centered; Session 1 vs.~Session 2), participant age (mean-centered), Method (dummy-coded, with central fixation as the reference level) and their interaction.
...
\hypertarget{s4.x.-moderator-analyses-including-a-more-restricted-sample}{%
\subsection{S4.X. Moderator analyses including a more restricted sample}\label{s4.x.-moderator-analyses-including-a-more-restricted-sample}}

\hypertarget{s4.x.1.-time-between-test-sessions}{%
\subsubsection{S4.X.1. Time between test sessions}\label{s4.x.1.-time-between-test-sessions}}
...
\hypertarget{s4.2.2.-language-background}{%
\subsection{S4.2.2. Language background}\label{s4.2.2.-language-background}}

\begin{figure}
\centering
\includegraphics{MB1T_supplement_files/figure-latex/unnamed-chunk-20-1.pdf}
\caption{\label{fig:unnamed-chunk-20}Infants' preference in Session 1 and Session 2 with individual data points and regression lines color-coded by method (CF, ET, or HPP). Results are plotted separately for North American English-learning infants (right panel) and infants learning other languages and dialects (right panel).}
\end{figure}
...
\hypertarget{s5.1.-log-transformed-looking-times}{%
\subsection{S5.1. Log-transformed looking times}\label{s5.1.-log-transformed-looking-times}}
...
\caption{\label{tab:unnamed-chunk-23}Coefficient estimates from a linear mixed-effects model predicting Log LT IDS preference in Session 2.}
...
{\centering \includegraphics[width=5in]{MB1T_supplement_files/figure-latex/unnamed-chunk-24-1} 

}

\caption{IDS preferences (based on average log-looking times) of both sessions plotted against each other for each inclusion criterion. n indicates the number of included infants, r is the Pearson correlation coefficient as the indicator for reliability.}\label{fig:unnamed-chunk-24}
\end{figure}
...
\hypertarget{s5.2.-proportion-looking-to-ids}{%
\subsection{S5.2. Proportion looking to IDS}\label{s5.2.-proportion-looking-to-ids}}
...
{\centering \includegraphics[width=5in]{MB1T_supplement_files/figure-latex/unnamed-chunk-26-1} 

}

\caption{IDS preferences (based on proportion IDS looking) of both sessions plotted against each other for each inclusion criterion. n indicates the number of included infants, r is the Pearson correlation coefficient as the indicator for reliability.}\label{fig:unnamed-chunk-26}
\end{figure}
...
\caption{\label{tab:unnamed-chunk-27}Coefficient estimates from a linear mixed-effects model predicting IDS preference (based on proportion IDS looking) in Session 2.}
...
\caption{\label{tab:unnamed-chunk-36}Linear mixed-effects model results predicting IDS preference in Session 2 from IDS preference in Session 1 at the stimulus level.}
